...
public class GeoRecommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		Set<String> visitedItemNames = new HashSet<>();
		for (Entry<String, Integer> category : categoryList) {
			List<Item> items = connection.searchItems(lat, lon, category.getKey());

			for (Item item : items) {
				if (!favoritedItemIds.contains(item.getItemId()) && !visitedItemIds.contains(item.getItemId()) && !visitedItemNames.contains(item.getName())) {
					recommendedItems.add(item);
					visitedItemIds.add(item.getItemId());
					visitedItemNames.add(item.getName());
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		// Step 1, get all favorited itemids
		DBConnection connection = DBConnectionFactory.getConnection();
		Set<String> favoritedItemIds = connection.getFavoriteItemIds("1111");
		...
		for (Entry<String, Integer> category : categoryList) {
			...
			List<Item> items = connection.searchItems(37.38, -122.08, category.getKey());
			Set<String> visitedItemIds = new HashSet<>();
			Set<String> visitedItemNames = new HashSet<>();
			for (Item item : items) {
				if (!visitedItemIds.contains(item.getItemId()) && !visitedItemNames.contains(item.getName())) {
					recommendedItems.add(item);
					visitedItemIds.add(item.getItemId());
					visitedItemNames.add(item.getName());
